...
class LILA(LDIMMethodBase):
    ...
    def __init__(self, modl: Union["LinearRegression","XGBRegressor","SVR"] = "LinearRegression"):
        super().__init__(
            name="lila",
            version="0.2.0",
    ...
    def _train(
        ...
        bar_nodes = manager.counter(
            total=len(nodes),
            desc='Pressures_node and Flow_sensor(Outer)',
            unit='Nodes',
            count=0
        )
        ...
        for i, node in enumerate(nodes):
            bar_nodes.update()
            # Create a counter for tracking the progress of the inner loop
            bar_nodes_inner = manager.counter(
                total=len(nodes),
                desc='Pressures_node and Flow_sensor(Inner)',
                unit='Nodes',
                count=0
            )
            X_tr = np.concatenate(
